...
// Matching behavior follows Skia's Fontconfig port.
#include "src/text/ports/font_manager_fontconfig.hpp"
...
// Match Skia's piecewise integer conversion, including demilight and book.
...
// after it. An entirely weak list is left intact, as in Skia's remove_weak.
...
struct FontConfigState {
  ...
  std::shared_ptr<Typeface> MakeTypeface(Pattern pattern) {
    ...
    // The pinned Skia port also leaves FC_FONT_VARIATIONS and named-instance
    // synthesis to future work. Ordinary collection indices are retained.
    auto real =
